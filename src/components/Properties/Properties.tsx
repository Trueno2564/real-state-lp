...
const loadLessHouses = () => {setCounterHouses(counterHouses-8)}
    return (
        <Transition className="px-4 my-8 md:py-32 md:px-40">
                    ...
                    <Link key={id} href={`/properties/${id}`} className="shadow-light hover:shadow-xl rounded-2xl transition-all duration-300 cursor-pointer">
                        <div className="relative -z-[1]">
                            <div className="relative">
                                ...
                                <Image src={`/assets/properties/${image}`} alt="Imagen Propiedad" width={150} height={150} className="object-cover w-full max-h-full h-[200px] rounded-2xl"/>
            ...
            <div className="text-center my-7">
                {counterHouses < dataProperties.length ? (
                    <button 
                        className="px-6 py-5 text-white transition-all duration-150 cursor-pointer bg-secondary rounded-xl hover:bg-black" onClick={loadMoreHouses}>
                        Ver mas viviendas
                    </button>
                ):
                    <button 
                        className="px-6 py-5 text-white transition-all duration-150 cursor-pointer bg-secondary rounded-xl hover:bg-black" onClick={loadLessHouses}>
                        Ver menos viviendas
                    </button>
                }
